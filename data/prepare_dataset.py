...
# Function for calculating the X4 kernel from the X2 kernel
...
def degradation(input_img, kernel, scale_factor, noise_im, device=torch.device('cuda')):
    # preprocess image and kernel
    input_img = torch.from_numpy(input_img).type(torch.FloatTensor).to(device).unsqueeze(0).permute(3, 0, 1, 2)
    input_img = F.pad(input_img, pad=(kernel.shape[0] // 2, kernel.shape[0] // 2, kernel.shape[0] // 2, kernel.shape[0] // 2),
                  mode='circular')
...
def generate_dataset(images_path, out_path_im, out_path_ker, k_size, scale_factor, min_var, max_var, noise_ker,
    ...
    for i, path in enumerate(files_source):
        ...
        if is_x4_analytic:
            # use analytic kernel calculated from x2.
...
def main():
    ...
    parser.add_argument('--model', type=str, default='EBC',
                        help='EBC, generate data blurred by anisotropic Gaussian kernel.')
